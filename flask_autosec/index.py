...
def check_internet_connection():
    ...
    if INTERNET_CONNECTION:
        logger.info(
            "Internet connection detected or user chose to continue without it."
        )
        ...
        async with aiohttp.ClientSession() as session:
            try:
                await session.post(
                    ...
                    json={"message": self.message, "type": "sec_violation"},
                )
...
class FlaskAutoSec:
    ...
    def _setup_routes(self, app):
        ...
        def check_blacklist_and_rate_limit():
            ...
            if (
                self.request_counts[ip]["count"] > self.current_rate_limit
                and self._enforce_rate_limits
            ):
                ...
                response.headers["Retry-After"] = str(
                    int(self.request_counts[ip]["reset_time"] - current_time)
                )
